/* This program reads a bitmap image file. It converts a
   colored bitmap image to a four-color filter 4-by-4 block bitmap image.
*/
#include <stdio.h>
#include <stdlib.h>
...
/* ==================================================================*/
  ...
  for (i = 0; i < header.Height; i++) { // Go through all rows.
    for (j = 0; j < header.Width; j++) { // Go through all pixels in row i.
      ...
      // Calculate the filter for the transformed pixel.
      // Filter value 0: lower-left, 1: lower-right, 2: upper-left, 3: upper-right.
      filter = i / (header.Height / 2) * 2 + j / (header.Width / 2);
      switch (filter) {
        case 0: {// lower-left: green-colored filter
          imageData[k+1] = 255;
          break;
        }
        case 1: { // lower-right: red-colored filter
          imageData[k+2] = 255;
          break;
        }
        case 2: {  // upper-left: gray-level filter
          imageData[k] = (imageData[k] * 28 + imageData[k+1] * 151 + imageData[k+2] * 77) / 256;
          imageData[k+1] = imageData[k];
          imageData[k+2] = imageData[k];
          break;
        }
        case 3: { // upper-right: blue-colored filter
          imageData[k] = 255;
          break;
        }
      }
  ...
  if (argc==3) {
    ...
    printf("The band filtered image is \"%s\".\n\n", argv[2]);
  } 
  else {
    fptr = fopen("blocked_filter.bmp", "wb");
    printf("The blocked-filtered image is \"blocked_filter.bmp\".\n\n");
  }  
