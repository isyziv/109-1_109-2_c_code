...
int main(int argc, char *argv[]) {
  ...
  create_list(&tokenList);
  cnt = 0;
  token = (unsigned char *) strtok(buffer, punctuation);
  
  while (token!=NULL) {
    if (search_list(tokenList, token)==NULL) {
      insert_list(&tokenList, token);
      cnt++;
    }  
    token = (unsigned char *) strtok(NULL, punctuation);
  }
  
  printf("File \"%s\" has %d tokens.\n\n", "source.txt", cnt);
