/* This program reads a bitmap image file. It performs mirror reflection along
   the horizontal direction and then attached the transformed image below the
   original image.
*/
#include <stdio.h>
#include <stdlib.h>
...
/* ==================================================================*/
// *** Allocate double size of memory to store the image data and the processed data.
  imageData = (unsigned char *) malloc(header.ImageSize * 2);
  fread(imageData, header.ImageSize, 1, fptr);
	
  fclose(fptr);
  
  // *** Copy the image to place two idential images stacked along the vertical direction. 
  for (i=0; i<header.ImageSize; i++) imageData[i+header.ImageSize] = imageData[i];
  ...
  // Update the bit map file header.
  header.Size = header.Size + header.ImageSize;
  header.Height = header.Height * 2;
  header.ImageSize = header.ImageSize * 2;
  
  /* Output to the horizontal inverted refelction image. */
  if (argc==3) {
    fptr = fopen(argv[2], "wb");
    printf("\nThe horizontal inverted reflection image is \"%s\".\n\n", argv[2]);
  } 
  else {
    fptr = fopen("inverted_reflection_horizontal.bmp", "wb");
    printf("\nThe horizontal inverted mirror reflectgion image is \"inverted_reflection_horizontal.bmp\".\n\n");
  }  
