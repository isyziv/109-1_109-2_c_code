...
int main(void) {
  ...
  // Place 1 at the center of column n-1.
  row = n / 2;
  col = n - 1;
  ...
  while (value<=n*n) {
    if (square[(row-1+n)%n][(col+1)%n]==0) {
      // Fill in the value on the upper-right.
      square[(row-1+n)%n][(col+1)%n] = value++;
      row = (row - 1 + n) % n;
      col = (col + 1) % n;
    }
    else {
      // Fill in the value the left. 
      square[row][(col-1+n)%n] = value++;
      col = (col - 1 + n) % n;
    }
  }
