...
int main(void) {
  ...
  // Place 1 at the center of column 0.
  row = n / 2;
  col = 0;
  square[row][col] = value++;

  while (value<=n*n) {
    if (square[(row+1)%n][(col-1+n)%n]==0) {
      // Fill in the value on the lower-left.
      square[(row+1)%n][(col-1+n)%n] = value++;
      row = (row + 1) % n;
      col = (col - 1 + n) % n;
    }
    else {
      // Fill in the value on the right.
      square[row][(col+1)%n] = value++;
      col = (col + 1) % n;
    }
  }
