...
int main(void) {
  ...
  // Place 1 at the center of row n-1.
  row = n - 1;
  col = n / 2;
  ...
  while (value<=n*n) {
    if (square[(row+1)%n][(col+1)%n]==0) {
      // Fill in the value on the lower-right.
      square[(row+1)%n][(col+1)%n] = value++;
      row = (row + 1) % n;
      col = (col + 1) % n;
    }
    else {
      // Fill in the value on the above.
      square[(row-1+n)%n][col] = value++;
      row = (row - 1 + n) % n;
    }
  }
