...
void printTerm(float value, char* str) {
  
  if (value > 0) { printf("+%6.4f%s", value, str); }
  else if (value < 0) { printf("%6.4f%s", value, str); }
}
...
int main(void) {
  ...
  if (equal0(quadratic_eval(c2, c1, c0, root1)) &&
    ...
    printf("%6.4f", root1.re);
    printTerm(root1.im, "i");
    printf(" and %6.4f", root2.re);
    printTerm(root2.im, "i");
    printf(" are the roots of equation %6.4fx**2", c2);
    printTerm(c1, "x");
    printTerm(c0, "");
    printf("=0.");
  }
    ...
    printf(" and %6.4f", root2.re);
    printTerm(root2.im, "i");
    printf(" are not the roots of equation %6.4fx**2", c2);
    printTerm(c1, "x");
    printTerm(c0, "");
    printf("=0.");
  }
